...
class BookingPage(tk.Frame): 
    ...
    def set_data(self, customer_id):
        ...
        query = """
            ...
            WHERE bk.customer_id = %s AND bk.cancelled_at IS NULL AND bk.booking_date > NOW() 
            ORDER BY bk.booking_date DESC
        """
        database.Database.cursor.execute(query, (self.customer_id,))
        future = database.Database.cursor.fetchall()
        ...
        database.Database.cursor.execute(query, (self.customer_id,))
        past = database.Database.cursor.fetchall()

        self.results = future + past
    ...
    def rate_service(self, booking_id, score):
        ...
        messagebox.showerror("Success", "Service rated successfully!")
    ...
    def cancel_booking(self, booking_id):
        ...
        messagebox.showerror("Success", "Booking cancelled successfully!")
    ...
    def show_service_page(self, service):
        ...
        query = """
            ...
            WHERE sv.id = %s
        """
        database.Database.cursor.execute(query, (self.customer_id, service[1]))
